...
- 🔄 **Triển khai nội dung code** (cập nhật 2026-07-13):
  ...
  - ✅ **Notebook 01 (`01_data_understanding.ipynb`) — HOÀN THÀNH** (T01, PR #12): 32 cell; tổng quan 8 bảng, phân tích `application_train` (307.511×122, `TARGET` ~8%, 67 cột thiếu), quan hệ khóa, từ điển dữ liệu. **Bổ sung diễn giải kỹ thuật (2026-07-16, PR #40):** thêm mục "Bài toán & phương pháp tiếp cận" (phát biểu bài toán, phân loại nhị phân có giám sát, phân biệt ROC-AUC vs accuracy), giải nghĩa thuật ngữ Anh–Việt, sơ đồ quan hệ 8 bảng dạng ASCII, diễn giải từng bước — phục vụ tiêu chí Y3/Y4. Đã Restart & Run All, execution_count 1→14 liền mạch.
  ...
  - 🟡 **Báo cáo & slide — MỚI BẮT ĐẦU** (cập nhật 2026-07-16): đã có **bản thảo Chương 1 Business Understanding** (`docs/Business_Understanding.docx`, PR #41 — bối cảnh nghiệp vụ Home Credit, vai trò AI hỗ trợ chuyên viên, phát biểu bài toán, ý nghĩa `TARGET`, lý do là phân loại nhị phân) + đã điền **trang bìa mẫu** (`docs/2. Mau tai lieu.docx`: tiêu đề + danh sách 5 thành viên). ⚠️ **Nhưng file NỘP trong `reports/` vẫn là mẫu trắng:** `reports/slide-du-an-nhom-01.pptx` md5 **TRÙNG KHÍT** `docs/3. Mau bao cao.pptx`; `reports/tai-lieu-du-an-nhom-01.docx` còn `<<TÊN ĐỀ TÀI>>` và "GVHD: Thầy Nguyễn Văn A"; `reports/images/` rỗng. Còn lại: **Chương 2–6 + slide**, và phải **ghép Chương 1 từ `docs/` sang file nộp `reports/`**. Đây là 2 trong 4 sản phẩm phải nộp (xem mục 2.2) và chiếm trọn tiêu chí Y4.
...
## 4. Việc tiếp theo (cập nhật 2026-07-16 — sau khi merge diễn giải NB01 PR #40 + Chương 1 whitepaper PR #41)
...
| **Whitepaper `.docx` 6 chương** | 🟡 **~15% — Chương 1 có bản thảo (`docs/`), file nộp `reports/` còn trắng** |
...
3. **Whitepaper `.docx` (6 chương) + slide `.pptx`** — 🔴 **rủi ro lớn nhất còn lại của dự án.** ✅ Chương 1 Business Understanding đã có bản thảo (`docs/Business_Understanding.docx`, PR #41). Còn lại: **Chương 2–6**, **16 slide**, và **ghép Chương 1 từ `docs/` vào file nộp `reports/tai-lieu-du-an-nhom-01.docx`**. Chương 4 (Mô hình hóa) viết được đầy đủ nhờ NB06 (AUC 0,7792, so sánh 4 mô hình, ý nghĩa ngưỡng quyết định); Chương 2/3/5 không phụ thuộc NB06.
