...
## 3. Trạng thái hiện tại (cập nhật 2026-08-03)
...
- **Cập nhật pipeline notebook (PR #77–#82, 2026-07-26 đến 2026-07-28):**
  ...
  - **NB04:** đã tổ chức lại phần phân nhóm biến thành 9 nhóm và EDA theo từng câu hỏi. Đã hoàn thành Nhóm 1 (nhân khẩu học, gia đình và công việc), Nhóm 2 (tài chính và khoản vay, gồm DTI/LTV) và Nhóm 3 (lịch sử tín dụng từ bảng phụ: sự hiện diện/quy mô lịch sử vay, dư nợ, chậm thanh toán và mức độ gần đây). Cần hoàn thiện Nhóm 4–9, đồng bộ nhận xét với output và Restart & Run All trước PR.
- **Triển khai nội dung code** (cập nhật 2026-07-25):
  ...
  - **Notebook 02 — Database Organization (PR #59, 2026-07-22):** dựng lại toàn bộ pipeline PostgreSQL theo checklist NB02 của thầy.
    ...
    - Mạch làm: tạo database/bảng → import bằng `COPY` → kiểm tra số dòng và kiểu dữ liệu → tạo index → **5 bảng summary** gom bảng phụ về mức khách hàng → `LEFT JOIN` tạo **`application_flat` 307.511 dòng × 154 cột** → validation → kết nối Python.
    ...
    - **Đã khai thác `bureau_balance` theo hai tầng (PR #69, 2026-07-24):** tổng hợp thành `bureau_balance_summary` theo `SK_ID_BUREAU`, rồi đưa 6 đặc trưng lịch sử trạng thái vào `bureau_summary` theo `SK_ID_CURR`. Bốn bảng summary còn lại được chuẩn hóa tên theo bảng nguồn; các câu truy vấn kiểm tra đã có output minh họa trong NB02 (PR #70).
  ...
  - **Chuẩn hóa cách trình bày NB01 (PR #71, 2026-07-25):** các mục lớn dùng số La Mã I–X; tách riêng phần đánh giá sơ bộ và kết luận; bảng nhận xét bảng chính/bảng phụ và bảng bàn giao được rút gọn để dễ đọc. Phần mở đầu NB01 cũng nhắc người đọc đọc `docs/Business_Understanding.docx` trước để nắm bối cảnh nghiệp vụ và mục tiêu bài toán.
  ...
  - **Bài học chung (2026-07-13):** NB02/04/05 đều dính lỗi kiểu *"chạy được máy tác giả, hỏng máy khác"* — đường dẫn cứng, thiếu thư viện trong `requirements.txt`, và **lệch phiên bản** (pandas 3.0/numpy 2.x vs bản cũ). Đã **ghim phiên bản thư viện** trong `requirements.txt` để cả nhóm chạy giống nhau.
...
## 4. Việc tiếp theo (cập nhật 2026-08-03)
...
|---|---|
| Mã nguồn `.ipynb` | **NB00–NB02 xong; NB03 cần Restart & Run All; NB04 đã xong EDA Nhóm 1–3, còn Nhóm 4–9; NB05–NB07 chưa xây dựng lại** |
...
2. **Hoàn thiện NB04 — EDA & Visualization:** tiếp tục Nhóm 4–9, rà soát insight/bàn giao sang NB05 và Restart & Run All để đồng bộ output. Không dùng lại EDA của pipeline cũ làm nguồn sự thật.
...
> Trạng thái thật: NB00, NB01 và NB02 đã có trên `main`; NB03 đã cập nhật tới PR #81 nhưng cần chạy xác minh lại; NB04 đã hoàn thành EDA Nhóm 1–3 và cần tiếp tục Nhóm 4–9; NB05–NB07 chưa xây dựng lại theo pipeline mới. Tài liệu hướng dẫn giảng viên, `README.md` và `requirements.txt` vẫn có sẵn.
