...
- **NB03 — Data Cleaning (đang sửa Mục III.1 Missing Values, 2026-08-12 — nhánh `fix/fix-NBs-by-Hung-09082026`):** Đã rút gọn phần giới thiệu. Trong hướng xử lý missing: (1) tạo 5 cờ lịch sử `has_bureau`, `has_previous`, `has_installments`, `has_pos_cash`, `has_credit_card` nhưng vẫn giữ `NaN` ở cột gốc; (2) với 6 cột `amt_req_credit_bureau_*`, tạo cờ `has_credit_bureau_request_info` trước khi điền `0`; (3) với xe, chỉ tạo `car_age_missing_when_owned` khi `flag_own_car = 'Y'` và `own_car_age` thiếu, sau đó điền `0` cho `own_car_age` — không tạo `has_car` vì trùng nghĩa với `flag_own_car`; (4) thêm assert kiểm tra hai hướng tạo cờ/điền `0`; (5) bổ sung assert cho hướng điền median và điền nhãn `"Missing"`; (6) chuyển `name_type_suite` sang điền `"Missing"`, không xóa khách; hướng xóa dòng còn 8 cột và đã thêm so sánh số dòng/tỷ lệ `TARGET` trước–sau cùng assert. **Chưa chạy lại notebook**, nên mọi output cũ chưa phản ánh code mới. Cần tiếp tục rà Mục III từ Duplicate trở đi, đồng bộ header input thành 164 cột, rồi Restart & Run All để kiểm tra output/nhận xét và bảng `application_flat_cleaned`.
