...
- **NB03 — Data Cleaning (hoàn thành nội dung, 2026-08-12 — nhánh `fix/fix-NBs-by-Hung-09082026`):** Đã đồng bộ Input `application_flat` thành 307.511 dòng × 164 cột. Missing được xử lý theo 4 hướng: tạo 5 cờ lịch sử nhưng giữ `NaN` ở cột gốc; tạo cờ rồi điền `0` cho 6 cột `amt_req_credit_bureau_*`; tạo cờ cho khách có xe nhưng thiếu tuổi xe rồi điền `0`; điền median/nhãn `"Missing"` hoặc xóa dòng thiếu ít tùy trường hợp, kèm các assert kiểm tra. Không giảm các cột `_avg`/`_mode`/`_medi` ở NB03 vì đây là quyết định feature selection cho NB05/NB06. Đã đổi thứ tự: Mục 3 xử lý dữ liệu sai logic trước, Mục 4 mới khảo sát 15 cột Outlier (thêm `bureau_sum_overdue`, `previous_approved_sum_credit`), cap P99 các cột có skewness từ 3 trở lên và minh họa tối đa 2 cột lệch mạnh nhất. `bureau_sum_debt` âm được sửa về `0`; `credit_card_avg_balance` âm được giữ vì có thể là trả thừa/hoàn tiền; `days_employed = 365243` được tạo cờ, thay bằng median; các dòng `code_gender = 'XNA'` được xóa. Đã nâng các heading rà soát lên cấp 5, bổ sung `object` khi ánh xạ kiểu pandas sang PostgreSQL và cập nhật kết luận. Trước khi tạo PR, cần Restart Kernel → Run All để làm mới output, xác nhận các assert, bảng Before/After và bảng `application_flat_cleaned` trong PostgreSQL.

- **NB06 — Machine Learning (2026-08-06 — nhánh `feat/hung-feat-NB06-06082026`):** Đã tạo header và bộ khung 9 mục cho `notebooks/06_modeling_evaluation.ipynb`. Đã hoàn thành ngắn gọn Mục I, II và III.1: đọc `application_features`, kiểm tra dữ liệu/`target`, tách `X` và `y`, loại `sk_id_curr` và `target` khỏi feature, có assert kiểm tra. Các cell này đã được chạy trong notebook. Mục III được rút gọn theo luồng dễ hiểu: xác định X/y → chia train/test → tạo lại interaction → xử lý missing → mã hóa → chuẩn hóa; bỏ phần rà soát insight riêng vì trừu tượng. Chưa làm III.2 trở đi, các mô hình và phần đánh giá.

- **Kiến thức NB06 từ clip ML (2026-08-06 — nhánh `feat/hung-feat-NB06-06082026`):** Đã lưu tóm tắt clip *Các mô hình Machine Learning phổ biến và cách đánh giá* vào `docs/huong-dan-giang-vien/video-bai-giang.md`. Nội dung chốt cho NB06: Home Credit là binary classification; Logistic Regression là baseline, Random Forest là mô hình so sánh; XGBoost chỉ dùng khi nhóm thống nhất dependency. NB06 phải chia train/test có `stratify`, chỉ fit preprocessing trên train, đánh giá ROC-AUC/PR-AUC/Precision/Recall/F1/Confusion Matrix, phân tích threshold và lưu model cùng metadata. Đã tạo header và bộ heading gồm 9 mục cho `notebooks/06_modeling_evaluation.ipynb`.
