...
| 4. Xử lý Missing Values | Xử lý **theo nhóm cột**, không từng cột: `*_count` → 0 + flag; numeric <30% → median; category → mode; `credit_card_*`/`ext_source_1`/`occupation_type` theo quyết định Mục 5. Kèm **bảng Cleaning Rule** (nhóm cột → phương pháp → lý do) | Hoàn thành | **Before/After**: bar/heatmap tỷ lệ missing trước vs sau |
...
| 6. Xử lý Outlier | Phát hiện bằng **IQR hoặc Z-score** cho cột tiền lệch phải (`amt_income_total`, `amt_credit`, `bureau_sum_credit`...). Xử lý bằng **biến đổi log hoặc cap theo phân vị, KHÔNG xóa dòng** (phần lớn là khách vay lớn có thật). Nêu rõ câu của thầy: "không phải outlier nào cũng nên xóa" | Hoàn thành | **Before/After**: boxplot + histogram cột tiền |
| 7. Xử lý sai logic | `days_employed = 365243` → NaN (rồi điền); `code_gender = 'XNA'` → gộp về nhóm phổ biến ('F'); giá trị âm `bureau_sum_debt`/`credit_card_avg_balance` → 0. Mỗi ca theo mạch Vấn đề→Bằng chứng→Xử lý→Lý do→Kiểm tra | Hoàn thành | **Before/After**: histogram `days_employed` (cột dựng ở 365243 biến mất), bar `code_gender` (XNA gộp đi) |
| 8. Loại cột trùng lặp / không cần thiết | Bỏ 28 cột `_medi`/`_mode` numeric nhà ở (quyết định Mục 5.3). Kiểm cột hằng số (constant) nếu có. Ghi bảng lý do từng nhóm cột bị bỏ | Hoàn thành | Bảng |
