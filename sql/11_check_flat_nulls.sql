...
-- null_mong_doi  : tinh tu chinh database = so khach cua applications
    ...
    SELECT
        ...
        ((SELECT COUNT(*) FROM applications)
         - (SELECT COUNT(*) FROM bureau_summary s
            JOIN applications a ON a.sk_id_curr = s.sk_id_curr))::BIGINT AS null_mong_doi
    UNION ALL
    SELECT
        ...
        ((SELECT COUNT(*) FROM applications)
         - (SELECT COUNT(*) FROM previous_application_summary s
            JOIN applications a ON a.sk_id_curr = s.sk_id_curr))::BIGINT
    UNION ALL
    SELECT
